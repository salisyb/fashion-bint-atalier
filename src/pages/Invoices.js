...
import { getInvoiceOrder, getListOfOrder } from "api/clients.api";
import { Send, FileDownload } from "@mui/icons-material";
...
export default function Invoice() {
  ...
  const i = new URLSearchParams(search).get("kbyb");
  ...
  const handleShrare = () => {
    shareOnMobile({
      text: "Invoice from bint atelier",
  ...
  const getOrders = async () => {
    ...
    const order = await getInvoiceOrder(i);
  ...
  if (orders.length === 0) {
    return (
      <Box
        ...
        py="200px"
      >
        <Typography>Invoice Link expire or not found</Typography>
      </Box>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <PDFDownloadLink
          document={<MyDocument data={orders} />}
          fileName="bint-test"
        >
          {({ loading }) =>
            loading ? (
              ...
              <Button variant="outlined" endIcon={<FileDownload />}>
                Download
              </Button>
            )
          }
